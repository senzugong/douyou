      ...
      select: function(){
        ...
        ,events = function(reElem, disabled, isSearch){
          ...
          var showDown = function(){
            ...
            notOption(input.val(), function(none){
              if(none){
                ...
                var value = dl.find('.'+THIS).attr('lay-value');
                input && input.val(value === '' ? '' :initValue);
              }
            });
          };
          ...
          dds.on('click', function(){
            ...
            select.val(value).removeClass('layui-form-danger'), input.val(value === '' ? '' : othis.text());
            ...
            ,'<dl class="layui-anim layui-anim-upbit'+ (othis.find('optgroup')[0] ? ' layui-select-group' : '') +'">'+ function(options){
              ...
              layui.each(options, function(index, item){
                if(index === 0 && !item.value) {
                  arr.push('<dd lay-value="'+ item.value +'" class="layui-select-tips '+ (value === item.value ?  THIS : '') + (item.disabled ? (' '+DISABLED) : '') +'">'+ (item.innerHTML ? item.innerHTML:TIPS) +'</dd>');
                  return;
                }
